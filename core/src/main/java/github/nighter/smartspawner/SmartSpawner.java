...
import github.nighter.smartspawner.emp.economy.EmpTransactionLogger;
import github.nighter.smartspawner.emp.economy.VaultEconomyProvider;
...
public class SmartSpawner extends JavaPlugin implements SmartSpawnerPlugin {
    ...
    private EmpTransactionLogger empTransactionLogger;
    ...
    public void onEnable() {
        ...
        getLogger().info("EMP has been enabled! (Loaded in " + loadTime + "ms)");
    }
    ...
    private void initializeEmpSystems() {
        this.empConfig = new EmpConfig(this);
        this.empConfig.load();

        if (databaseManager == null || !databaseManager.isActive()) {
            getLogger().severe("EMP economy requires database storage. Set database.mode to SQLITE or MYSQL.");
            getServer().getPluginManager().disablePlugin(this);
            return;
        }

        this.empDatabase = new EmpDatabase(this, databaseManager);
        if (!empDatabase.initialize()) {
            getLogger().severe("Failed to initialize EMP database tables.");
            getServer().getPluginManager().disablePlugin(this);
            return;
        }

        boolean txLogging = empConfig.getConfig().getBoolean("economy.transaction_logging", true);
        this.empTransactionLogger = new EmpTransactionLogger(this, databaseManager, txLogging);
        this.empAccountService = new EmpAccountService(this, empConfig, databaseManager, empTransactionLogger);
    ...
    public void onDisable() {
        saveAndCleanup();
        SpawnerMobHeadTexture.clearCache();
        getLogger().info("EMP has been disabled!");
    }

    private void saveAndCleanup() {
        if (gemRewardService != null) {
            gemRewardService.stop();
        }
    ...
    private void setupVaultEconomyProvider() {
        if (empConfig == null) {
            return;
        }

        if (!empConfig.getConfig().getBoolean("economy.vault_provider.enabled", true)) {
            return;
        }

        if (getServer().getPluginManager().getPlugin("Vault") == null) {
            getLogger().warning("Vault not found. EMP economy will not register a Vault provider.");
            return;
        }

        boolean override = empConfig.getConfig().getBoolean("economy.vault_provider.override", true);
        RegisteredServiceProvider<Economy> existing = getServer().getServicesManager().getRegistration(Economy.class);
        if (existing != null && !override) {
            getLogger().info("Vault economy provider already exists. EMP provider registration skipped.");
            return;
        }

        String singular = empConfig.getConfig().getString("economy.currency.singular", "Coin");
        String plural = empConfig.getConfig().getString("economy.currency.plural", "Coins");

        this.vaultEconomyProvider = new VaultEconomyProvider(empAccountService, empAccountService.getMoneyFormat(), singular, plural);
        getServer().getServicesManager().register(Economy.class, vaultEconomyProvider, this,
                override ? ServicePriority.Highest : ServicePriority.Normal);
        getLogger().info("EMP Vault economy provider registered.");
    }
